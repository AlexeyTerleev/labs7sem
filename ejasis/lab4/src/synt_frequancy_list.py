...
def get_list(text: str, english: bool):
    stop_words = stopwords.words("english" if english else "russian")
    analize_func = analyze_english_word if english else analyze_russian_word
    ...
    for index in range(len(word_tokens)):
        word_tokens[index] = [word_tokens[index], word_tokens.count(word_tokens[index])]
    ...
    results_list = [analize_func(token) for token in res_word_tokens]
    return results_list
